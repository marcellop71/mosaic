...
func BIGtoBig(x *BIG) *big.Int {
	x_, _ := new(big.Int).SetString(x.ToString(), 16)
  return x_
}

func BigToBIG(x_ *big.Int) *BIG {
	b := x_.Bytes()
	z := make([]byte, int(MODBYTES) - len(b))
	x := FromBytes(append(z, b...))
	//fmt.Println(">>> %s %s", x_.Text(16), x.ToString())
  return x
}
