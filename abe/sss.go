...
func swapCol(a *[][]int, j0 int, j1 int) {
  for i := 0; i < len((*a)[0]); i++ {
    tmp0 := (*a)[j0][i]; tmp1 := (*a)[j1][i]
    (*a)[j0][i] = tmp1; (*a)[j1][i] = tmp0
  }
}
...
func hermiteNormalForm(a [][]int) ([][]int, [][]int) {
  ...
  for i, k := m - 1, n - 1; i >= q; i-- {
    ...
    if (w != 0) {
      swapCol(&a_, j0, k); swapCol(&u, j0, k)
      if (row(a_, i)[k] < 0) {
        negCol(&a_, k); negCol(&u, k)
      }
      for j := 0; j < k; j++ {
        tmp := nearestint(a_[j][i], a_[k][i])
        lcCol(&a_, j, k, tmp); lcCol(&u, j, k, tmp)
      }
    } else {
      if (row(a_, i)[k] < 0) {
        negCol(&a_, k); negCol(&u, k)
